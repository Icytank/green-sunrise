...
- [x] Define Project Schema (AC: 1, 2, 3, 5)
  - [x] Modify `src/content/config.ts` to include the `projects` collection
  - [x] Implement `z.object` with strictly typed fields
  - [x] Create a `TerrainType` enum to maintain data consistency
- [x] Initialize Content Directory (AC: 4)
  - [x] Create `src/content/projects/bg` and `src/content/projects/en`
  - [x] Add sample project files with valid data placeholders
- [x] Technical Validation (AC: 5)
  - [x] Run `npx astro check` to verify schema enforcement
  - [x] Verify that `capacityMW` only accepts numeric input
...
- Created `TerrainType` enum with values: 'Mountainous', 'Flat', 'Industrial', 'Roof-top'
- Updated projects collection schema with: title, capacityMW (number), location, terrainType (enum), date, heroImage (image), isVerified (boolean)
- Created bilingual sample projects: solar-park-plovdiv (Flat, 10.5 MW), industrial-rooftop-sofia (Roof-top, 2.8 MW)
- Schema enforcement verified via `npx astro check`

### File List

- src/content.config.ts (modified)
- src/content/projects/bg/solar-park-plovdiv.md (new)
- src/content/projects/en/solar-park-plovdiv.md (new)
- src/content/projects/bg/industrial-rooftop-sofia.md (new)
- src/content/projects/en/industrial-rooftop-sofia.md (new)

## Change Log

- 2026-01-21: Implemented project content collection with TerrainType enum, capacityMW number field, and bilingual sample content
